# <h1 align = "center"> Lista de Exercícios: Python - Treinamento de Laços de Repetição for e while </h1>

<div align = "center">

<img src = ".\imgs\foto-capa-readme.png" alt = "Imagem mostrando o Código do Exercício 6" heigh = "400">

</div>

### Status da Lista: Concluída ✔️ <br>

## 🗂 Sumário

- [📝 Descrição da Lista](https://github.com/Victor-M-S-Rodrigues07/Python__Treinamento_de_Condicionais_If_Elif_e_Else__Lista_de_Exercicios_N1_2026#-descri%C3%A7%C3%A3o-da-lista)
- [🎯 Objetivos da Lista](https://github.com/Victor-M-S-Rodrigues07/Python__Treinamento_de_Condicionais_If_Elif_e_Else__Lista_de_Exercicios_N1_2026#-objetivos-da-lista)
- [💻 Tecnologias Utilizadas](https://github.com/Victor-M-S-Rodrigues07/Python__Treinamento_de_Condicionais_If_Elif_e_Else__Lista_de_Exercicios_N1_2026#-tecnologias-utilizadas)
- [💪🏻 Exercícios](https://github.com/Victor-M-S-Rodrigues07/Python__Treinamento_de_Condicionais_If_Elif_e_Else__Lista_de_Exercicios_N1_2026#-exerc%C3%ADcios)
- [📂 Estrutura da Lista](https://github.com/Victor-M-S-Rodrigues07/Python__Treinamento_de_Condicionais_If_Elif_e_Else__Lista_de_Exercicios_N1_2026#-estrutura-da-lista)
- [✨Autor](https://github.com/Victor-M-S-Rodrigues07/Python__Treinamento_de_Condicionais_If_Elif_e_Else__Lista_de_Exercicios_N1_2026#autor)
- [👨🏻‍💻 Contatos](https://github.com/Victor-M-S-Rodrigues07/Python__Treinamento_de_Condicionais_If_Elif_e_Else__Lista_de_Exercicios_N1_2026#%E2%80%8D-contatos--refer%C3%AAncias)

## 📝 Descrição da Lista

Esta lista contém 10 (dez) exercícios relacionados a laços de repetição, mais especificamente laços *for* e *while*. Laços de repetição são um dos recursos lógicos mais utilizados no mundo da programação. Essas estruturas estão presentes na maioria das aplicações desenvolvidas atualmente. Aprender e desenvolver essas habilidades por meio de situações práticas do cotidiano, e não apenas na teoria, é fundamental para consolidar o aprendizado. Assim, a resolução desses problemas permitiu este autor aprofundar sua compreensão sobre os laços de repetição e suas aplicações em diferentes contextos. <br>

Todos os exercícios propostos tiveram origem no curso *Praticando Python: Condicionais if, elif e else*, da Alura (Link no final do documento), enquanto as soluções apresentadas são de autoria deste desenvolvedor e compartilhados para fins acadêmicos e profissionais <br> <br>

## 🎯 Objetivos da Lista

- Revise o conceito de laços for e while em Python;
- Entenda a diferença entre os dois tipos de laços;
- Pratique a codificação de laços.

## 💻 Tecnologias Utilizadas

![Python](https://img.shields.io/badge/Python-0?style=for-the-badge&logo=python&logoColor=%23FFFFFF&color=%233776AB)
![Git](https://img.shields.io/badge/Git-0?style=for-the-badge&logo=git&logoColor=%23FFFFFF&color=%23F05032)
![GitHub](https://img.shields.io/badge/GitHub-0?style=for-the-badge&logo=github&logoColor=%23FFFFFF&color=%23181717) <br>

## 🧩 Exercícios

**1.	Compreendendo laços ➰;**\
**2.	O que é um loop infinito? ♾;**\
**3.	Quantas vezes a mensagem será exibida? 📨;**\
**4.	Calculando a soma de números ➕;**\
**5.	Organizando seu portfólio 🗂;**\
**6.	Entendendo o uso do break 🛑;**\
**7.	Controle de estoque 📦;**\
**8.	Contagem Regressiva 🎆;**\
**9.	Utilidade do continue em laços ⏭;**\
**10.	Validação de entrada para login ✅.**

## 📂 Estrutura da Lista

```
/Exercicios
    Exercicio-1.py
    Exercicio-2.py
    Exercicio-3.py
    Exercicio-4.py
    Exercicio-5.py
    Exercicio-6.py
    Exercicio-7.py
    Exercicio-8.py
    Exercicio-9.py
    Exercicio-10.py

/img
    foto-capa-readme.png

README.md
```

## ✨Autor

<img src = "https://avatars.githubusercontent.com/u/187053289?v=4" width = 120px> <br>
<strong> Victor </strong>

## 👨🏻‍💻 Contatos & Referências

<a href = "https://www.linkedin.com/in/victor-m-rodrigues/"><img alt="Static Badge" src="https://img.shields.io/badge/LinkedIn-0?style=for-the-badge&logoColor=%23FFFFFF&color=%230077B5&link=https%3A%2F%2Fwww.linkedin.com%2Fin%2Fvictor-m-rodrigues%2F"></a> 
<a href = "https://github.com/Victor-M-S-Rodrigues07/"><img alt="Static Badge" src="https://img.shields.io/badge/GitHub-0?style=for-the-badge&logo=github&logoColor=%23ffffff&labelColor=%23000000&color=%2300000000"></a>
<a href = "https://cursos.alura.com.br/user/victorvicmr"><img alt="Static Badge" src="https://img.shields.io/badge/Alura-0?style=for-the-badge&color=%23100D36"></a>
<a href = "https://cursos.alura.com.br/course/praticando-python-lacos-for-while"> <img alt="Static Badge" src="https://img.shields.io/badge/Praticando%20Python%3A%20La%C3%A7os%20for%20e%20while-0?style=for-the-badge&label=Curso&labelColor=444444&color=F29999">
 </a>

🇧🇷 - 2026
